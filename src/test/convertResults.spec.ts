...
describe("convertResults", () => {
  it("should convert a Spectral report into Codacy results", async () => {
    ...
    const expected: ToolResult[] = [
      new Issue("example.json", "The provided document does not match any of the registered formats [OpenAPI 2.0 (Swagger), OpenAPI 3.x, OpenAPI 3.0.x, OpenAPI 3.1.x, AsyncAPI 2.x]", "unrecognized-format", 1),
      new Issue("example.json", "Invalid symbol", "parser", 1),
    ]
